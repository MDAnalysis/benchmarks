...
asv run -e -j 4 --config ${ASV_CONFIG} NEW || true
...
## For Python 2.7: start at release-0.11.0 --merges
## For Python 3.6: start at release-0.17.0 --merges
## For Python 3.8: start at release-1.0.1 --merges
## For Python 3.11: start at release-2.4.0 --merges
## For Python 3.11 & 3.14: start at release-2.8.0 (every commit)
#
# used to only benchmark --merges; however, we often squash-merge and there is no clear merge commit
# so since 2.8.0 we are running every commit
timeout ${TIMEOUT} asv run -e -j 4 --config ${ASV_CONFIG} "release-2.8.0..HEAD" --skip-existing || true
